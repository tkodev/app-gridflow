...
export async function savePostMutationFn(
  vars: SavePostMutationInput
): Promise<Post> {
  ...
  if (isEditing && post) {
    const currentIds = new Set(
      mediaItems.filter((m) => !m.isNew).map((m) => m.id)
    );
...
export async function deletePostMutationFn(
  vars: DeletePostMutationInput
): Promise<void> {
  const supabase = createClient();
...
export async function reorderPostsMutationFn(
  vars: ReorderPostsMutationInput
): Promise<void> {
  const supabase = createClient();
