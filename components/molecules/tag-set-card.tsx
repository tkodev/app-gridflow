...
const styles = {
  root: cva(
    'bg-card group/tag-set-card hover:bg-surface-container-low flex cursor-pointer items-center gap-3 rounded-xl p-3 shadow-[0_2px_16px_-2px_hsl(var(--foreground)/0.04)] transition-colors'
  ),
  ...
  chip: cva('bg-surface-container-low rounded-full px-2 py-0.5 text-xs font-medium'),
...
const TagSetCard: React.FC<TagSetCardProps> = (props) => {
  ...
  // c. logic
  const tags = tagSet.tags.split(/\s+/).filter(Boolean).slice(0, 5)
